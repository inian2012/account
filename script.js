document.addEventListener('DOMContentLoaded', () => {
    ...
    function parseAndAddData(rawText) {
        ...
        lines.forEach(line => {
            ...
            if (parts.length > 0) {
                // Default structure based on user request: 邮箱|密码|辅助邮箱|Token|国家
                const card = {
                    id: 'card_' + generateId(),
                    raw: line,
                    parsed: {
        ...
        if (addedCount > 0) {
            saveData();
            // Reset to beginning to show new cards
            state.currentCardIndex = 0;
            render();
        ...
        cards.forEach((card, index) => {
            ...
            cardEl.innerHTML = `
                ...
                <div class="card-actions">
                    ...
                    <button class="delete-card-btn" data-id="${card.id}"><i class="fa-solid fa-trash"></i> 删除</button>
                </div>
            `;
